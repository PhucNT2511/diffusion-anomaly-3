...
class EncoderUNetModel(nn.Module):
    ...
    def forward(self, x, timesteps):
        ...
        :param timesteps: a 1-D batch of timesteps.
        :return: an [N x K] Tensor of outputs. - K = 2 (2 neuron đầu ra, nhưng chưa softmax)
        """
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))
...
class MixedUNetModel(nn.Module):
    """
    The UNet model with attention and timestep embedding. 
    Using for both classification and segmentation.
    For usage, see UNet.
    """
    ...
    def __init__(
        ...
        #self.gap = nn.AvgPool2d((8, 8))  #global average pooling chả để làm qq gì
        #self.cam_feature_maps = None ### ko rõ để làm qq gì

        if pool == "adaptive": ### Adaptive Average từ (B, 256, H, W) --> (B,256,1,1) --> (B,2)
        ...
        elif pool == "spatial": ### Chỉ là linear: [B,256] -> [B,2], nhưng phải đảm bảo chỉ có 256 channels
            print('spatial')
            self.out_cls = nn.Linear(256, self.out_channels)

        elif pool == "spatial_v2": ### [B,C] --> [B,2048] --> [B,2]
            self.out_cls = nn.Sequential(
                nn.Linear(self._feature_size, 2048),
                normalization(2048),
                nn.SiLU(),
        ...
        self.out_segment = nn.Sequential(
            normalization(ch),
            nn.SiLU(),
            zero_module(conv_nd(dims, model_channels, segment_out_channels, 3, padding=1)),
        )
    ...
    def forward(self, x, x0, timesteps):
        """
        Apply the model to an input batch.
        ...
        :return: an [N x K] Tensor of outputs. - K = 2 (2 neuron đầu ra, nhưng chưa softmax)
        """
        t0 = th.zeros_like(timesteps)
        emb0 = self.time_embed(timestep_embedding(t0, self.model_channels))
        ...
        for module in self.input_blocks:
            ...
            ### ko dùng đến
            if self.pool.startswith("spatial"):
                results.append(h.type(x.dtype).mean(dim=(2, 3))) ### mean(dim=(2,3)) là globalAvg --> Mỗi block đều lấy trung bình như vậy --> [B,C]


        h = self.middle_block(h, emb)
        h0 = self.middle_block(h0, emb0)

        for module in self.output_blocks:
            h0 = th.cat([h0, h0_s.pop()], dim=1)
            h0 = module(h0, emb0)
        h0 = h0.type(x.dtype)
    

        if self.pool.startswith("spatial"): ## ko dùng đến
            results.append(h.type(x.dtype).mean(dim=(2, 3))) #--> [B, C1+C2+...+Cn]
            h = th.cat(results, axis=-1)         
        else:
            h = h.type(x.dtype)
