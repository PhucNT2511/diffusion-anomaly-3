...
def eval_iter(metric_tracker, model, x, y, ids, iteration, epoch, set_name, final_pred,final_prob, final_label):
    ...
    inputs = x.to(device) ##################### X
    targets = torch.tensor(y, dtype=torch.long).to(device) ###################### Y
...
if __name__ == "__main__":
    ...
    with tqdm.tqdm(initial=start_epoch, total=args.max_epochs) as epoch_pbar:
        ...
        for epoch in range(start_epoch, args.max_epochs):
            ...
            ################################## task_name = 'HEALTHY_NO_HEALTHY', only 1 in task_dict
            for task_name in task_dict.keys():
                is_best[task_name] = False
                ################ Assess if epoch_metric[f"{task_name}_accuracy_mean"] doesn't have any element, initialize 0, else add the last
